...
import {assMinutes, getMinutes, setMinutes, differenceInSeconds, addSeconds } from 'date-fns'
// import formatDate, { formatRequestData, formatGetTime } from '@utils/format-date'
import formatData,{ formatRequestData, formatGetTime } from '@utils/format-date' 
...
export default {
  ...
  computed: {
    ...
    mirrorWidth() {
      return this.splitLabels.length * 300
    },
  },
  watch:{
    selectedDateString(){
  ...
  methods: {
    ...
    // },
    prepaidListAfterService(){
      const id = this.submitData.params[0].proceId
      return this.filteredAvailablePrepaidList.filter(item =>{
        return item.prices.some(i =>{
          return id === i.id
        })
...
<template>
  <Layout>
    <!-- <BaseTableSearch v-slot="{ fields }" :default-value.sync="filterData" :border="true" :hide-reset="changingBooking" @reset="tryToFetchSchedule" @tryToFetchList="tryToFetchSchedule"> -->
    <BaseTableSearch v-slot="{ fields }" :default-value.sync="filterData" :border="true" :hide-reset="changingBooking" @reset="tryToFetchSchedule" @tryToFetchList="tryToFetchSchedule">
      ...
      <el-form-item v-show="!changingBooking" prop="orderStatus">
        <el-select v-model="fields.orderStatus">
          ...
          <el-option label="所有預約狀態" value></el-option>
          <el-option v-for="(status,value) in filterStatusLidt" :key="value" :label="status.title" :value="status.orderStatus"></el-option>
    ...
    <el-card v-loading.fullscreen.lock="tryingToGetOrder" element-loading-text="取得訂單資訊中" shadow="never" style="margin-bottom:0" :class="[$style['card-wrapper'], { [$style['mask']]: tryingFetchSchedule }]">
      <div class="vue-cal-layout" :style="changingBooking ? 'padding-bottom: 85px;' : ''">
        <div>
          ...
          <p :class="$sryle.status" style="margin-top:20px;margin-bottom:5px">預約狀態</p>
          <div :class="$style['calendar-status-wrapper']">
        ...
        <div class="schedule-wrap">
          <VueCal
            ...
            :class="[$style.schedule, $style[`split-labels-${staffs}`], { [$style['is-changing']]: changingBooking }]"
            active-view="day"
            locale="zh-hk"
            :selected-date="selectedDate"
            :time-from="6 * 60"
          ...
          <VueCal
            ref="cal"
            class="full-cal vuecal--full-height-delete"
            :class="[$style.schedule, $style[`split-label-${staffs}`],{ [$style['is-change']]: changeBooking }]"
            active-view='day'
            localw="zh-hk"
    ...
    >
    <el-button
      v-if="defaultValue.status !== 0 && 
      defaultValue.paymentStatus === 'UNPAID' &&
      defaultValue.paymentMethod !== 'Credit' &&
      defaultValue.paymentMethod !== 'Vacc' && 
      defaultValue.paymentMethos !== 'Prepaid'"
      class="is-round orderformSection__bottom__btn-item"
      type="success"
      :disabled="tryingStore"
      :loading="tryingStore"
    ...
    <el-radio-group v-model="paymentWay">
      ...
      <el-radio v-if="prepaidListAfterService.length > 0 && !isBuyPrepaid && !isPrepaidAndEdit && ordertype === 'prepaid-order'" label="Prepaid" border>
        包月抵扣
      </el-radio>
      <el-radio v-if="prepaidListAfterService.length > 0 && !isBuyPrepaid && !isPrepaidAndEdit && ordertype === 'prepaid-order'" label="Prepaid" border>
        包月抵扣
      ...
      <el-radio label="offline" :disabled="ordertype === 'prepaid-order' && !isBuyPrepaid && !isPrepaidAndEdit" border>線下付款
      <el-radio label="offline" :disabled="ordertype === 'prepaid-order' && !isBuyPrepaid && !isPrepaidAndEdit" border >線下付款</el-radio>
    ...
    <el-drawer
      ref="orderForm" 
      class="only-close"
      :visable.sync="createOrderVisible"
      :append-to-body = "true"
      :size="$root.isSmallDevice ? '100%' : '90%'"
      destroy-on-close
      :modal-append-to-body = "false"
      ...
      .vuecal__event {
        ...
        .wrap {
          ...
          &.info {
            ...
            background-color:lighten($color-status-info,30%)
          }
          &.is-disabled {
            pointer-events: none;
            &::before {
