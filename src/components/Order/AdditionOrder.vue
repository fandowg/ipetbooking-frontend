...
export default {
  components: {
    ...
    AdditionForm: () => lazyLoadComponent(import('@components/Order/AdditionFormOrder')),
    AdditionEditForm: () => lazyLoadComponent(import('@components/Order/AdditionEditFormOrder')),
...
<template>
  <div :class="$style.addition">
    <div v-if="defaultValue.addOrders" style="margin-bottom: 50px">目前無加購訂單</div>
    ...
    <el-drawer ref="drawer" class="no-header mobile" :visible.sync="drawerVisible" append-to-body :size="$root.isSmallDevice ? '100%' : '50%'" destroy-on-close @closed="handleDrawerClosed">
      ...
      <AdditionForm v-if="!isEditMode" :pet-size-id="petSizeId" :default-value.sync="defaultValue" :schedules="schedules" @update="updateDefaultValue" @close="handleDrawerClosed" @scrollToTop="scrollToTop" />
      <AdditionEditForm
        v-else
